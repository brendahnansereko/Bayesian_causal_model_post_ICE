...
#setwd("C:/Users/Brendah/OneDrive - London School of Hygiene and Tropical Medicine/Doctoral_training/causal_model_2/scripts/Stan/foundation/mmrm_discont_missing_data/")
# Parameters
...
for (j in 1:iter) {
  ...
  n_mcar <- ceiling(length(discont_treatment_ids) * 0.90)
  ...
  n_mcar <- ceiling(length(discont_ctrl_ids) * 0.90)
  #set.seed(123)  # Reproducible random selection
  ...
  yVec <- as.matrix(data[, c("Y0", "Y1", "Y2", "Y3", "Y4", "Y5")])
  ...
  datForStan = list(
    ...
    W = treatVec,
    discont_index = discontInd,#discontinuation indicator
    miss_index = missInd,#missing data indicator
    discontonly_index = discontonlyInd,#discontunation only indicator
    nondiscontmiss_index = nondiscontmissInd,#non miss non discontinuation indicator
...
fit <- stan(file = "/home/lsh1901704/stan_beta_5visits_miss_xx.stan", data = datForStan, 
            iter = 1000, chains = 1,
