...
for (j in 1:iter) {
  ...
  n_mcar <- ceiling(length(discont_treatment_ids) * 0.2)
  ...
  n_mcar <- ceiling(length(discont_ctrl_ids) * 0.2)
  #set.seed(123)  # Reproducible random selection
  ...
  M <- 50  # number of imputations per bootstrap
  ...
  #  set.seed(1000 + b)  # optional, for reproducibility
    ...
    t_indiv <- datForStan$t_indiv
        ...
        theta_imp <- numeric(M)  # store estimates from each imputed dataset
    
    for (m in 1:M) {
      y_pred <- yVec  # reset predictions each time
      ...
      for (n in 1:nrow(data_boot)) {
        ...
        if (n %in% missInd && post_n > 0) {
          ...
          Sigma_post <- Sigma_22 - Sigma_21 %*% Sigma_11_inv %*% Sigma_12
          ...
          # draw a fresh imputation
          y_pred[n, (t_n + 1):5] <- mvrnorm(1, mu = as.vector(mu_post), Sigma = Sigma_post)
        }
      ...
      theta_imp[m] <- coef(lm_fit)[2]
    }
    
    # Store the mean estimate across 10 imputations
    theta_boot[b] <- mean(theta_imp)
...
output_csv <- paste0("results_high_boot20_mi_batch_", batch, ".csv")
